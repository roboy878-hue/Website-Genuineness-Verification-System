...
class ScoreEngine:
    ...
    def _determine_verdict(self, score, results):
        ...
        # Check if domain is unresolvable (could be fake or legitimate unreachable in current network)
        ip_error = ip_result.get('error', '').lower()
        content_error = content_result.get('error', '').lower()
        ssl_error = ssl_result.get('error', '').lower()
        
        is_unresolvable = (
            ('could not resolve' in ip_error or 'getaddrinfo failed' in ip_error) or
            ('failed to resolve' in content_error or 'getaddrinfo failed' in content_error) or
            ('getaddrinfo failed' in ssl_error)
        )
        
        if is_unresolvable and ip_result.get('ip_address') is None:
            # First check: if domain is a trusted education/organization type, don't auto-flag as malicious
            domain_lower = results.get('parsed_domain', '').lower() if isinstance(results.get('parsed_domain'), str) else ''
            trusted_extensions = ['.edu', '.gov', '.ac.', '.org.', '.nic.']
            is_trusted_type = any(ext in domain_lower for ext in trusted_extensions)
            
            if is_trusted_type:
                logger.warning(f"[VERDICT LOGIC] Unresolvable trusted domain type (.edu/.gov/etc) - Maybe network issue, let score decide")
                # Don't auto-flag, let the weighted score determine verdict
            else:
                # For non-trusted domains, if they can't resolve AND SSL has errors = likely fake
                if ssl_result.get('score', 50) <= 20:
                    logger.error(f"[VERDICT LOGIC] Unresolvable domain + SSL error - MALICIOUS")
                    return ('Malicious', 'Dangerous')
                else:
                    logger.warning(f"[VERDICT LOGIC] Unresolvable domain - MALICIOUS")
                    return ('Malicious', 'Dangerous')
